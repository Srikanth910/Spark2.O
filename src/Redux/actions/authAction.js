...
export const signupCheckmobile = data => {
  ...
  return async dispatch => {
    try {
      ...
      if (createUserDetails.code === '313') {
        dispatch({
          type: SIGNUP_USER_FAIl,
          payload: createUserDetails,
        });
      } else if (createUserDetails.Data.code === '200') {
        dispatch({
          type: SIGNUP_USER_SUCCESS,
          payload: createUserDetails.Data,
        });
      }
    } catch (err) {
      console.log(err);
  ...
  return async dispatch => {
    try {
      const res = await axios.post(
        `${API_URL}/checkmobilenootpverification2_0 `,
        data,
      );
...
export const createMemberToken = data => {
  ...
  return async dispatch => {
    try {
      ...
      if (membarTokenDetails.code === '313') {
        dispatch({
