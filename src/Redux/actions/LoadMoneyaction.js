...
import {
  GET_RAZORPAY_DETAILS_SUCCESS,
  GET_RAZORPAY_DETAILS_FAIL,
  CATACH_ERROR,
  GET_CARD_SUCCESS,
  ...
  return async dispatch => {
    try {
      const res = await axios.get(
        `${API_URL}/getRazorpayVADetails?memberId=${
          data.memberId
        }&isFirsttime=false `,
      );
...
export const addSavingsAccountBalanceRazorpay = data => {
  ...
  return async dispatch => {
    try {
      const res = await axios.get(
        `${API_URL}/addSavingsAccountBalanceRazorpay?membarId=${
          data.membarId
        }&balance=${
          data.balance
        }&chargeAmount=0&isServiceChargePayedByCust=no&isShareBuy=false&pgTransId=${
          data.pgTransId
        }`,
      );
